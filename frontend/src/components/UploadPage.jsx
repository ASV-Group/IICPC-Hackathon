...
export default function UploadPage({ userToken }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile.size > 50 * 1024 * 1024) {
      setUploadStatus({ success: false, message: 'File is too large! Maximum limit is 50MB.' });
      return;
    }
  ...
  const handleUploadSubmit = async (e) => {
    e.preventDefault();
    if (!file) {
      setUploadStatus({ success: false, message: 'Please select a file to submit.' });
      return;
    }

    setLoading(true);
    setUploadStatus({ success: null, message: 'Uploading and scanning binary...' });

    const formData = new FormData();
    formData.append('submission_file', file);

    try {
      const response = await fetch('http://localhost:3000/api/v1/submissions/submit', {
        method: 'POST',
        headers: {
      ...
      setUploadStatus({ success: false, message: 'Network connection lost. Server is unreachable.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="upload-container">
      <div className="upload-card">
        ...
        <p className="upload-subtitle">Upload statically compiled Linux executables or ZIP files containing your codebase.</p>
        ...
        <form onSubmit={handleUploadSubmit} className="upload-form">
          <div className="drag-drop-zone" onClick={() => fileInputRef.current.click()}>
            ...
            {file ? (
              <p className="selected-file-name">Selected: <strong>{file.name}</strong></p>
            ) : (
              <p>Drag & Drop your compiled binary or Click to browse.</p>
            )}
            <span className="file-limits">ZIP or executable file limits up to 50MB.</span>
          </div>
        ...
        {uploadStatus.message && (
          <div className={`status-banner ${uploadStatus.success === true ? 'success' : uploadStatus.success === false ? 'error' : 'info'}`}>
            {uploadStatus.message}
          </div>
        )}
      </div>
